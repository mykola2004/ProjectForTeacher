...
1.  **Size of the tribe**: The tribe starts with a fixed even number $N$ of individuals in the initial generation. The number of women and men is the same. The age of all of them does not exceed 10 years. The tribe is isolated from other tribes.
...
4.  **Offspring**:
    1.  **Reproductive age**: A woman can become pregnant up to age $q_F$, and a man can father a child up to age $q_M$. The parameters $q_F$ and $q_M$ are determined by the user.
...
-   `marriage_related` - Indicator of related marriage.

## Algorithm steps {.unnumbered}

-   **Initiation**
    1.  Initialize tables `TD`, `TD_Birth`, `TD_Death` with columns `id`, `sex`, `parents`, `year_birth`, `defect`, `age`, `age_death`, `marriage`, `marriage_year`, `marriage_related`.
    2.  Generate a population of size `N` consisting of an equal number of men and women aged from 0 to 10 years. Save the population to the `TD`.
    3.  Set `year` to the maximum age in the population.
-   **Loop** <br/>
    1.  Increase `year` by 1.
    2.  Update the age of each individual in the `TD`.
    3.  If an individual has reached the age of death, save them to the `TD_Death` and remove them from the `TD`.
    4.  If the birth year in the `TD_Birth` is equal to `year`, add the newborn individuals to the `TD` and remove them from the `TD_Birth`.
    5.  Check if new marriages can be formed. If not, go to step 1.
    6.  Form new marriages and save them to the `TD`. For each newly formed pair, save `marriage_year` as the current year.
    7.  For each newly formed pair:
        1.  Check if the pair is related up to `K_G` generations back. If so, set `marriage_related` to `True`, otherwise set it to `False`.
        2.  Draw the number of offspring from a Poisson distribution with parameter `lambda_par`, provided that both partners are in childbearing age and a break between births is maintained.
        3.  For each offspring:
            1.  Draw the year of birth considering the interval between births `d_B`.
            2.  Draw the year of death according to the normal distribution of lifespan for the given sex (female: `mu_F, s2_F`, male: `mu_M, s2_M`).
            3.  Draw the occurrence of a genetic defect with probability `p_D` for offspring from related parents and `p_P` for offspring from a parent with a genetic defect.
            4.  Add the newborn offspring to the `TD_Birth`.
    8.  If `year` is less than `T`, go to step 1.
    9.  Return the `TD`, `TD_Birth`, `TD_Death`.
...
-   Women and men without genetic defects: <svg width="50" height="30"> <g transform="translate(10,10)"> <path d="M0,-10A10,10 0 1,1 0,10A10,10 0 1,1 0,-10" fill="#FFE0BD"></path> <path d="M 0,-12.408065 10.745699,6.2040324 -10.745699,6.2040324 Z" fill="#FFE0BD" transform="translate(30,2)"></path> </g> </svg>
-   Women and men with genetic defects: <svg width="50" height="30"> <g transform="translate(10,10)"> <path d="M0,-10A10,10 0 1,1 0,10A10,10 0 1,1 0,-10" fill="#D3D3D3"></path> <path d="M 0,-12.408065 10.745699,6.2040324 -10.745699,6.2040324 Z" fill="#D3D3D3" transform="translate(30,2)"></path> </g> </svg>
-   Deceased women and men: <svg width="50" height="30"> <g transform="translate(10,10)"> <path d="M0,-10A10,10 0 1,1 0,10A10,10 0 1,1 0,-10" fill="#FFE0BD" stroke="#000000" stroke-width=0.8></path> <path d="M 0,-12.408065 10.745699,6.2040324 -10.745699,6.2040324 Z" fill="#FFE0BD"  stroke="#000000" stroke-width=0.8 transform="translate(30,2)"></path> </g> </svg>
-   Unrelated pair: <svg width="50" height="30"> <g transform="translate(10,10)"> <path d="M -7.0710678118654755,-7.0710678118654755 h 14.142135623730951 v 14.142135623730951 h -14.142135623730951Z" fill="orange"></path> </g> </svg>
-   Related pair: <svg width="50" height="30"> <g transform="translate(10,10)"> <path d="M -7.0710678118654755,-7.0710678118654755 h 14.142135623730951 v 14.142135623730951 h -14.142135623730951Z" fill="#D3D3D3"></path> </g> </svg>
-   Indication of individuals forming a pair: <svg width="50" height="30"> <line x1="5" y1="10" x2="45" y2="10" stroke="orange" stroke-width="3" /> </svg>
-   Offspring of a married couple: <svg width="50" height="30"> <line x1="5" y1="10" x2="45" y2="10" stroke="red" stroke-width="1" /> </svg>
